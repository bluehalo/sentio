	...
	describe('Creation', function() {
		...
		context('when complete', function() {
			...
			it('should not throw an error', function() {
				(function() {
					controller = sentio.controllerRealtimeBins({
						binCount: 10,
						binSize: 1
					});
				}).should.not.throw();
...
// Just a dummy file to ensure something is emitted
...
describe('Extent', function() {
	...
	describe('Usage', function() {
		context('when configured with a default value', function() {
			var extentController = sentio.modelExtent({
				defaultValue: [ 0, 1 ]
			});
