...
var l=G.x.domain(),c=t(O,P,E.valueX).filter(function(t){
...
// Join
var t=X.g.markers.selectAll(".marker").data(D,E.markerValueX),e=t.enter().append("g").attr("class","marker").on("mouseover",function(t,e){I.call("markerMouseover",this,t,e)}).on("mouseout",function(t,e){I.call("markerMouseout",this,t,e)}).on("click",function(t,e){I.call("markerClick",this,t,e)}),n=e.append("line"),r=e.append("text");n.attr("y1",function(t){return G.y.range()[1]}).attr("y2",function(t){return G.y.range()[0]}),r.attr("dy","0em").attr("y",-3).attr("text-anchor","middle").text(E.markerValueLabel);
// Enter + Update
...
var V=z.merge(s);V.transition().duration(p).style("opacity","1").attr("transform",function(t,e){return"translate("+h.left+","+(h.top+r*e)+")"}),
...
var E=V.selectAll("rect.cell").data(function(t){return k.map(function(e,n){return{key:m.key(e,n),value:t.getValue(e,n)}})},m.key);/*
		 * Cell Remove
		 */
return E.enter().append("rect").attr("class","cell").style("opacity","0.1").style("fill",function(t,e){return x.color(t.value)}).attr("x",function(t,e){return x.x(t.key)+g}).attr("y",g).attr("height",f).attr("width",f).on("mouseover",l).on("mouseout",o).on("click",d).merge(E).transition().duration(p).style("opacity","1").attr("x",function(t,e){return x.x(t.key)+g}).style("fill",function(t,e){return x.color(t.value)}),E.exit().transition().duration(p).attr("width",0).style("opacity","0.1").remove(),A},A.data=function(t){return arguments.length?(k=null!=t?t:[],A):k},A.series=function(t){return arguments.length?(w=null!=t?t:[],A):w},A.cellSize=function(t){return arguments.length?(f=t,A):f},A.cellMargin=function(t){return arguments.length?(g=t,A):g},A.margin=function(t){return arguments.length?(h=t,A):h},A.duration=function(t){return arguments.length?(p=t,A):p},A.key=function(t){return arguments.length?(m.key=t,A):m.key},A.colorScale=function(t){return arguments.length?(x.color=t,A):x.color},A.xScale=function(t){return arguments.length?(x.xScale=t,y.x.scale(t),A):x.xScale},A.yScale=function(t){return arguments.length?(x.yScale=t,A):x.yScale},A.xExtent=function(t){return arguments.length?(v.x=t,v.x.getValue(function(e,n){return t(e,n)}),A):v.x},A.valueExtent=function(t){return arguments.length?(v.value=t,A):v.value},A.dispatch=function(){return z},A},t.chartTimeline=f,t.chartAutoBrushTimeline=function(){function t(t){var e=t;
